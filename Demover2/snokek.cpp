...
Point food;
Point food2;
SDL_Texture* food2Texture = NULL;
const int NUM_BARRIERS = 10;
Point barriers[NUM_BARRIERS];
...
void placeFood(Snake& snake) {
    ...
    bool onSecondFood = true;
    while (onSecondFood) {
        food2.x = rand() % (SCREEN_WIDTH / GRID_SIZE);
        food2.y = rand() % (SCREEN_HEIGHT / GRID_SIZE);

        onSecondFood = false;
        for (const auto& segment : snake.segments) {
            if (food2.x == segment.x && food2.y == segment.y) {
                onSecondFood = true;
                break;
            }
        }
...
void initializeBarriers(Snake& snake) {
    for (int i = 0; i < NUM_BARRIERS; i++) {
        bool onBarrier = true;
        while (onBarrier) {
            barriers[i].x = rand() % (SCREEN_WIDTH / GRID_SIZE);
            barriers[i].y = rand() % (SCREEN_HEIGHT / GRID_SIZE);

            onBarrier = false;
            for (const auto& segment : snake.segments) {
                if (barriers[i].x == segment.x && barriers[i].y == segment.y) {
                    onBarrier = true;
                    break;
                }
            }
            for (int j = 0; j < i; j++) {
                if (barriers[i].x == barriers[j].x && barriers[i].y == barriers[j].y) {
                    onBarrier = true;
                    break;
                }
            }
...
int updateSnake(Snake& snake) {
    ...
    if (newHead.x < 0 || newHead.x >= SCREEN_WIDTH / GRID_SIZE || newHead.y < 0 || newHead.y >= SCREEN_HEIGHT / GRID_SIZE) {
        ...
        initializeBarriers(snake);
        return 1;
    }
    for (int i = 0; i < NUM_BARRIERS; i++) {
        if (newHead.x == barriers[i].x && newHead.y == barriers[i].y) {
            initializeGame(snake);
            initializeBarriers(snake);
            return 3;
        }
    ...
    if ((newHead.x == food.x && newHead.y == food.y)||(newHead.x == food2.x && newHead.y == food2.y)) {
...
void renderGame(Snake& snake) {
    ...
    for (int i = 0; i < NUM_BARRIERS; i++) {
        SDL_Rect rBarrier = { barriers[i].x * GRID_SIZE, barriers[i].y * GRID_SIZE, GRID_SIZE, GRID_SIZE };
        SDL_RenderCopy(gRenderer, barrierTexture, NULL, &rBarrier);
    }
    SDL_Rect rFood = { food.x * GRID_SIZE, food.y * GRID_SIZE, GRID_SIZE, GRID_SIZE };
    SDL_RenderCopy(gRenderer, foodTexture, NULL, &rFood);
    SDL_Rect rFood2 = { food2.x * GRID_SIZE, food2.y * GRID_SIZE, GRID_SIZE, GRID_SIZE };
    SDL_RenderCopy(gRenderer, food2Texture, NULL, &rFood2);
...
bool setUpThing()
{
    ...
    food2Texture = loadTexture("banana.png", gRenderer);
    barrierTexture = loadTexture("bridge.png", gRenderer);
